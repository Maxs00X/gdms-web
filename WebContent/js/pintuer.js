...
function queren(msg, $tr){
	var d = dialog({
		...
		content : msg,
		ok : function(){
			console.log("此处开始逻辑代码");
			var goodsid = $tr.children("td:eq(0)").children("input").val(); //id
			var param = {
				"goodsid" : goodsid
			};
			$.ajax({
				url : "admin/goods/delsingle.php" ,
				type : "get" ,
				data : param ,
				success : function (r){
					console.log("id =>" + goodsid + " delete success");
					$tr.remove();
					var s = dialog({
						content: r.msg,
					});
					s.showModal().width(200);
					setTimeout(function () {
						s.close().remove();
					}, 2000);
				}

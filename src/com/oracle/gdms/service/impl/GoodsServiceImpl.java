...
import com.oracle.gdms.dao.GoodsTypeDao;
...
//		String url = AppListener.getString("pushurl_self");
		String url = "http://localhost:8080/rest/huazhao/push/goods/one";
//		String url = "http://amsou54172.java.cdnjsp.org/rest/huazhao/push/goods/pushgood";
		...
		try {
			System.out.println("要推送的数据：" + jsonstr);
	...
	@Override
	public List<GoodsModel> findByDesc(String[] params) {
		session = GDMSUtil.getSession();
		GoodsDao dao = session.getMapper(GoodsDao.class);
//		GoodsDao gdao = session.getMapper(GoodsDao.class);
//		GoodsTypeDao gtdao = session.getMapper(GoodsTypeDao.class);
		Map<String,Object> m = new HashMap<>();
		m.put("desc", params);
//		List<Integer> gtidlist = null;
		List<GoodsModel> list = null;
		try{
			list = dao.findByDesc(m);
//			gtidlist = gtdao.findByDesc(m);
//			list = gdao.findByDesc2(gtidlist);
		}catch (Exception e){
			e.printStackTrace();
		}finally {
			free();
